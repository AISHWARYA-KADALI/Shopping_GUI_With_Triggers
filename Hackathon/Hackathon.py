...
class Image_Module:
    ...
    def rooticon(self,root):
        ...
        p1 = ImageTk.PhotoImage(Image.open(r"E:\Icons\shopping.jfif"))#(r'C:\Users\admin\Desktop\shopping\category\shopping1.jpg')) 
...
class Admin_Module:
    ...
    def ViewForm(self):
        obj = Admin_Module()
        viewform = Toplevel()
        viewform.geometry("700x380")
        viewform.resizable(0, 0)
        TopViewForm = Frame(viewform,width=600, bd=1, relief=SOLID)
        TopViewForm.pack(side=TOP, fill=X)
        LeftViewForm = Frame(viewform,width=600)
        LeftViewForm.pack(side=LEFT, fill=Y)
        MidViewForm = Frame(viewform,width=700,height=400)
        ...
        btn_notify = Button(LeftViewForm, text="Notofication", command=lambda: obj.Notification())
        btn_notify.pack(side=TOP, padx=10, pady=10, fill=X)
    ...
    def Notification(self):
        obj = Admin_Module()
        viewform = Toplevel()
        viewform.geometry("650x320")
        #viewform.resizable(0, 0)
        TopViewForm = Frame(viewform,width=600, bd=1, relief=SOLID)
        TopViewForm.pack(side=TOP, fill=X)
        MidViewForm = Frame(viewform,width=900,height=500)
        MidViewForm.pack(side=BOTTOM)
        lbl_text = Label(TopViewForm, text="Order Details(Notifications)", font=('arial', 18), width=600)
        lbl_text.pack(fill=X)
        ...
        scrollbarx.config(command=tree.xview)
        scrollbarx.pack(side=BOTTOM, fill=X)
        tree.heading("1st", text="DELIVERY TIME")
        tree.heading("2nd", text="DISTRIBUTOR ID")
        tree.heading("3rd", text="PRODUCT ID")
        tree.heading("4th", text="MESSAGE(STATUS)")
        '''tree.heading("5th", text="A_PRICE")
        tree.heading("6th", text="S_PRICE")
        tree.heading("7th",text="QUANTITY")'''
        tree.column('#0', stretch=NO, minwidth=0, width=0)
        ...
        tree.column('#7', stretch=NO, minwidth=0, width=100)'''
        tree.pack()
        obj.DisplayData_Notification(tree)

    def DisplayData_Notification(self,tree):
        cursor.execute("select * from notification order by DELIVERED_TIME desc,MSG")
        fetch = cursor.fetchall()
        for data in fetch:
            tree.insert('', 'end', values=(data))
    def Update(self,tree):
        ...
        if not tree.selection():
            ...
            root = Tk()
            frame = Frame(root, bg='skyblue')  # bg='pink',
            ...
            Button(frame, text="Update Product ", font=(12), width=25, activebackground="pink", activeforeground="blue",
                   command=lambda: productprice(A_price, S_price, selecteditem[0])).grid(pady=25, row=12, columnspan=4)          
        root.mainloop()
    ...
    def add(self,frame, pro, Ap, Sp, dtls, brd, cat, pro_img, quantity):
        ...
        if (no == 0):
            if (flag1 == 1 and flag2 == 1):
                ...
                def open():
                    filename = filedialog.askopenfilename(initialdir="E:/", title="Select A file",
    ...
    def add_product(self):
        ...
        def open():
            filename = filedialog.askopenfilename(initialdir="E:/", title="Select A file", filetypes=(("all files", "*.*"),
...
class Customer_Module:
        def search_frame(self,root):
            ...
            search = ImageTk.PhotoImage(Image.open(r"E:\Icons\search.png"))
            cart = ImageTk.PhotoImage(Image.open(r"E:\Icons\cart1.jpg"))
            menu = ImageTk.PhotoImage(Image.open(r"E:\Icons\menu.png"))
            order = ImageTk.PhotoImage(Image.open(r"E:\Icons\order.png"))
...
photo= ImageTk.PhotoImage(Image.open(r"E:\Icons\shoppingicon.png"))
key = Login()
cart_id = 0
cust_id = 0
